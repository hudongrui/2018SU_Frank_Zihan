...
class model:
    ...
    def initial_weights(self, weight_file='./models/weights/bvlc_alexnet.npy'):
        if weight_file:
            ...
            fc7b_init = tf.constant(0.1, shape=[1024])
            fc8W_init = tf.truncated_normal(
                [1024, self.THETA_SIZE], stddev=0.1)
    ...
    def gen_model(self, image_batch):
        ...
        s_h = 4
        s_w = 4
        conv1_in = conv(image_batch, self.conv1W, self.conv1b,
        ...
        s_w = 1
        group = 2
        conv2_in = conv(maxpool1, self.conv2W, self.conv2b, k_h,
                        k_w, c_o, s_h, s_w, padding="SAME", group=group)
        ...
        s_w = 1
        group = 1
        conv3_in = conv(maxpool2, self.conv3W, self.conv3b, k_h,
                        k_w, c_o, s_h, s_w, padding="SAME", group=group)
        ...
        s_w = 1
        group = 2
        conv4_in = conv(conv3, self.conv4W, self.conv4b, k_h,
                        k_w, c_o, s_h, s_w, padding="SAME", group=group)
        ...
        s_w = 1
        group = 2
        conv5_in = conv(conv4, self.conv5W, self.conv5b, k_h,
                        k_w, c_o, s_h, s_w, padding="SAME", group=group)
    ...
    def gen_loss(self, fc8, theta_label_batch, grasp_label_batch):
        ...
        # computiong batch size as an op from the output
        input_batch_size = tf.gather(fc8_shape, 0)
        theta_one_hot = tf.one_hot(
            theta_label_batch, depth=18, on_value=1.0, off_value=0.0)
        theta_acted = tf.reduce_sum(
            fc8 * theta_one_hot, reduction_indices=1, name='theta_acted')
        sig_op = tf.clip_by_value(tf.nn.sigmoid(
            theta_acted), 0.001, 0.999, name='clipped_sigmoid')
        ...
        conf = tf.equal(tf.to_int32(tf.greater_equal(sig_op, 0.5)), tf.to_int32(tf.greater_equal(
            grasp_label_batch, 0.1)))  # the plus 0.5 is there because of the alpha added on because of shake
        self.conf = conf
