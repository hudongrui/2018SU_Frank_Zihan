...
class grasp_obj:
    ...
    def test_init(self):
        tf.reset_default_graph()  # 12/19/2017 added by BW
        with tf.device(self.dev_name):
            with tf.name_scope('Grasp_training_data'):
                self.Grasp_patches = tf.placeholder(tf.float32, shape=[
                                                    self.BATCH_SIZE, self.IMAGE_SIZE, self.IMAGE_SIZE, self.NUM_CHANNELS])
            with tf.name_scope('Grasp'):
                self.M = grasp_net()
                self.M.initial_weights(weight_file=None)
                self.grasp_pred = self.M.gen_model(self.Grasp_patches)
        with tf.device("/cpu:0"):
            grasp_variables = tf.get_collection(
                tf.GraphKeys.GLOBAL_VARIABLES, scope='Grasp')
    ...
    def test_one_batch(self, Is):
        with tf.device(self.dev_name):
            grasp_feed_dict = {self.Grasp_patches: Is,
                               self.M.dropfc6: 1.0, self.M.dropfc7: 1.0}
