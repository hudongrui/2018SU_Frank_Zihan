...
class Predictors:
    ...
    def graspNet_grasp(self, init_loc, num_angle=18, patch_size=300, num_samples=128):
        ...
        patch_hs = np.random.randint(
            h_range, size=num_samples) + half_patch_size
        patch_ws = np.random.randint(
            w_range, size=num_samples) + half_patch_size
        # print 'initialized patch_hs is: ', patch_hs
        # print 'initialized patch_ws is: ', patch_ws

        patch_Is = np.zeros((num_samples, patch_size, patch_size, self.I_c))

        print "patch_Is initialized with shape", patch_Is.shape
        # print "patch_Is initialized as", patch_Is

        patch_Is_resized = np.zeros(
            (num_samples, self.learner.IMAGE_SIZE, self.learner.IMAGE_SIZE, self.I_c))
        for looper in xrange(num_samples):
            ...
            while isWhiteFlag == 1:
                ...
                patch_hs[looper] = init_loc[1] + np.random.randint(low=-10, high=10, size=1)
                patch_ws[looper] = init_loc[0] + np.random.randint(low=-10, high=10, size=1)
                patch_Is[looper] = self.I[patch_hs[looper] - half_patch_size:patch_hs[looper] - half_patch_size +
                                                                             patch_size,
                                   patch_ws[looper] - half_patch_size:patch_ws[looper] - half_patch_size + patch_size]
            ...
            patch_Is_resized[looper] = cv2.resize(
                patch_Is[looper], (self.learner.IMAGE_SIZE, self.learner.IMAGE_SIZE), interpolation=cv2.INTER_CUBIC)
        # subtract mean
        ...
        for looper in xrange(num_samples):
            for norm_looper in xrange(num_angle):
                self.fc8_norm_vals[looper, norm_looper] = (wf[1] * self.fc8_norm_vals[looper, norm_looper] +
                                                           wf[0] * self.fc8_norm_vals[
                                                               looper, (norm_looper - 1) % num_angle] +
                                                           wf[2] * self.fc8_norm_vals[
                                                               looper, (norm_looper + 1) % num_angle])
